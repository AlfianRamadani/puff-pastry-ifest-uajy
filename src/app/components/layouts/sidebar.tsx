...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Mobile Bottom Bar */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 w-full flex z-50 h-18 bg-black border-t-[3px] border-black">
        {navItems.map((item) => {
          const isActive =
            item.href === '/'
              ? pathname === '/'
      ...
      {/* Desktop Sidebar */}
      <div className="hidden md:flex flex-col w-64 min-h-screen bg-black border-r-[4px] border-black font-sans">
        {/* Brand */}
        <div className="p-5 border-b-[3px] border-black">
          <div className="flex items-center gap-3 bg-[#FFC107] border-[3px] border-black p-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all">
            <div className="bg-black w-10 h-10 flex items-center justify-center">
              <Zap className="w-6 h-6 text-[#FFC107]" strokeWidth={3} fill="#FFC107" />
            </div>
        ...
        <div className="p-4 border-t-[3px] border-white/10">
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-8 h-8 bg-[#FFB3C1] border-2 border-white/30 flex items-center justify-center">
              <span className="font-black text-xs text-black">A</span>
            </div>
            <span className="font-bold text-xs text-white/50 uppercase tracking-wide">Student</span>
          </div>
        </div>
      </div>
    </>
